...
public abstract class UserCreator {
    ...
    public static User createAdmin(String passwd){
        ...
        DataBaseHelper.addUserToBase(user, successful -> {
            if(successful){
                DataBaseHelper.updateUserDataInBase(user,ignoringListener);
            }
        });
        return user;
    }
    public static Student createStudent(String fio, Date birthDate, StudentGroup group){
        ...
        DataBaseHelper.addUserToBase(student, successful -> {
            DataBaseHelper.updateUserDataInBase(student, ignoringListener);
        });
        return student;
    }
    ...
    public static User retrieveUserFromDoc(HashMap<String,Object> doc){
        ...
        try{
            ...
            if(type == User.UserType.ADMIN)
                ...
                user = retrieveStudentFromDoc(passwd,doc);
    ...
    private static Student retrieveStudentFromDoc(String passwd,HashMap<String,Object> doc){
        Student student;
        try{
            ...
            student = new Student(passwd,fio,birthDate, group);
        }catch (Exception ex){
            return null;
        }
        return student;
